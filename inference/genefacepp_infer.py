...
class GeneFace2Infer:
    def __init__(self, torso_model_dir='checkpoints/motion2video_nerf/', head_model_dir='',
    ...
    def infer_once(self, inp):
        with torch.no_grad():
            logger.debug(f"开始推理: {inp['out_name']}")
    ...
    def prepare_batch_from_inp(self, inp):
        ...
        sample['wav16k_name'] = wav16k_name
        ...
        # ngp_pose: 与位姿相关的参数，通常是 3D 或 6D 位姿信息。
        # euler: 欧拉角，描述物体旋转的角度（通常为绕 X、Y、Z 轴的角度）。
        # trans: 平移向量，表示物体的平移位置。
        # self.dataset.poses: 数据集中的位姿集合，可能是每帧的 3D 位姿。
        # self.dataset.ds_dict['euler']: 数据集中的欧拉角列表。
        # self.dataset.ds_dict['trans']: 数据集中的平移列表。
        pose_lst = []
        euler_lst = []
        trans_lst = []
        rays_o_lst = []
        ...
        # 在每次循环中，都会根据不同的条件，选择对应的位姿（pose）和变换（transformation）参数。
        for i in range(t_x // 2):
            if inp['drv_pose'] == 'static':
                ngp_pose = self.dataset.poses[0].unsqueeze(0)  # 确保它是一个二维张量（通过 unsqueeze(0)）
    ...
    def forward_audio2secc(self, batch, inp=None):
        ...
        zero_eulers = torch.zeros([identity.shape[0], 3]).to(identity.device)
        zero_trans = torch.zeros([identity.shape[0], 3]).to(exp.device)
        if inp['debug']:
            with torch.no_grad():
                chunk_size = 50
                drv_secc_color_lst = []
                num_iters = len(identity) // chunk_size if len(identity) % chunk_size == 0 else len(identity) // chunk_size + 1
                ...
                for i in range(num_iters):  # 或使用其他适合的循环形式
                    ...
                    face_mask, drv_secc_color = self.secc_renderer(identity[i * chunk_size:(i + 1) * chunk_size],
            ...
            _, src_secc_color = self.secc_renderer(identity[0:1], exp[0:1], zero_eulers[0:1], zero_trans[0:1])
            _, cano_secc_color = self.secc_renderer(identity[0:1] * 0, exp[0:1] * 0, zero_eulers[0:1], zero_trans[0:1])
        ...
        if self.postnet_model is not None:
            ...
            b, t, *_ = pitch.shape
            raw_pred_lm3d = self.face3d_helper.reconstruct_idexp_lm3d(identity, exp).reshape([t, -1, 3])
            ...
            feat_fuse, _, _ = compute_LLE_projection(feats=idexp_lm3d[:, :68 * 3],
                                                     feat_database=idexp_lm3d_ds_lle[:, :68 * 3], K=10)
            ...
            idexp_lm3d = self.face3d_helper.reconstruct_idexp_lm3d(identity, exp)
        ...
        cano_lm3d = (idexp_lm3d_mean + idexp_lm3d_std * idexp_lm3d_normalized) / 10 + self.face3d_helper.key_mean_shape[
            index_lm68_from_lm478].unsqueeze(0)
        eye_area_percent = self.opened_eye_area_percent * torch.ones([len(cano_lm3d), 1], dtype=cano_lm3d.dtype,
                                                                     device=cano_lm3d.device)

        if inp['blink_mode'] == 'period':
            cano_lm3d, eye_area_percent = inject_blink_to_lm68(cano_lm3d, self.opened_eye_area_percent,
                                                               self.closed_eye_area_percent)
            logger.info("Injected blink to idexp_lm3d by directly editting.")
        batch['eye_area_percent'] = eye_area_percent
        idexp_lm3d_normalized = ((cano_lm3d - self.face3d_helper.key_mean_shape[index_lm68_from_lm478].unsqueeze(
            0)) * 10 - idexp_lm3d_mean) / idexp_lm3d_std
        idexp_lm3d_normalized = torch.clamp(idexp_lm3d_normalized, min=lower, max=upper)
        ...
        lm2d = self.face3d_helper.reconstruct_lm2d_nerf(identity, exp, smo_euler, smo_trans)
    ...
    def forward_secc2video(self, batch, inp):
        wav16k_name = batch['wav16k_name']
        ...
        if inp['low_memory_usage']:
            # save memory, when one image is rendered, write it into video (节省内存，渲染一个图像时，将其写入视频)
                ...
                for i in tqdm.trange(num_frames, desc="GeneFace++ is rendering... "):
                    with tqdm_lock:
                        model_out = self.secc2video_model.render(rays_o[i], rays_d[i], cond_inp[i], bg_coords, poses[i],
                                                                 index=i, staged=False, bg_color=bg_color,
                                                                 lm68=lm68s[i],
            ...
            if inp['debug']:
                ...
                secc_img = torch.cat([torch.nn.functional.interpolate(drv_secc_colors[i:i + 1], (512, 512)) for i in
                                      range(num_frames)]).cpu()
            ...
            logger.debug(
                f"=== ImageIO is saving video using FFMPEG(h264) to {tmp_out_name} 开始处理数据 ===: {inp['out_name']}...")
    ...
    def example_run(cls, inp=None):
        inp_tmp = {
            'drv_audio_name': 'data/raw/audios/zozo.wav',
        ...
        infer_instance = cls(inp['torso_ckpt'], inp['head_ckpt'], inp['a2m_ckpt'], inp['postnet_ckpt'])
...
def get_arg(torso_ckpt=None, head_ckpt=None):
    ...
    parser.add_argument("--drv_audio", default='data/raw/audios/MacronSpeech.wav')
