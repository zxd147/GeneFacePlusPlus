...
class RADNeRFTask(BaseTask):
    ...
    def build_model(self):
        ...
        self.embedders_params += [p for k, p in self.model.named_parameters() if
                                  p.requires_grad and 'position_embedder' in k]
        self.embedders_params += [p for k, p in self.model.named_parameters() if
                                  p.requires_grad and 'ambient_embedder' in k]
        self.network_params = [p for k, p in self.model.named_parameters() if (
                    p.requires_grad and 'position_embedder' not in k and 'ambient_embedder' not in k and 'cond_att_net' not in k)]
    ...
    def run_model(self, sample, infer=False):
        ...
        bg_color = sample['bg_torso_img'] if 'bg_torso_img' in sample else sample[
            'bg_img']  # treat torso as a part of background
        ...
        if not infer:
            ...
            model_out = self.model.render(rays_o, rays_d, cond_inp, bg_coords, poses, index=idx, staged=False,
                                          bg_color=bg_color, perturb=True, force_all_rays=False, cond_mask=cond_mask,
                                          eye_area_percent=eye_area_percent, **hparams)
            ...
            if self.model.training:
                ...
                losses_out['weights_entropy_loss'] = torch.mean(
                ...
                if hparams.get("ambient_loss_mode", 'mae') == 'mae':
                    losses_out['ambient_loss'] = (ambient.abs() * (~face_mask.view(-1))).sum() / (
                                (~face_mask).sum() + 1)
                else:
                    losses_out['ambient_loss'] = (ambient.pow(2) * (~face_mask.view(-1))).sum() / (
                                (~face_mask).sum() + 1)
            ...
            if self.global_step >= hparams['lpips_start_iters']:
                ...
                losses_out['sr_lip_lpips_loss'] = self.criterion_lpips(
                    sr_pred_rgb[:, :, xmin * 2:xmax * 2, ymin * 2:ymax * 2],
                    gt_rgb_512[:, :, xmin * 2:xmax * 2, ymin * 2:ymax * 2]).mean()
            ...
            if self.global_step >= hparams['lpips_start_iters'] and hparams['lambda_dual_fm'] > 0:
                ...
                losses_out['dual_feature_matching_loss'] = self.dual_feature_matching_loss_fn(fake_feature_maps,
                                                                                              real_feature_maps)
            ...
            model_out = self.model.render(rays_o, rays_d, cond_inp, bg_coords, poses, index=idx, staged=False,
                                          bg_color=bg_color, perturb=False, force_all_rays=True, cond_mask=cond_mask,
                                          eye_area_percent=eye_area_percent, **hparams)
            # calculate val loss
            if 'gt_img' in sample:
                gt_rgb = sample['gt_img'].reshape([1, 256, 256, 3]).permute(0, 3, 1, 2)
    ...
    def _training_step(self, sample, batch_idx, optimizer_idx):
        ...
        loss_weights = {
            ...
            'sr_lpips_loss': 0.5 * hparams['lambda_lpips_loss'] if self.global_step >= hparams[
                'lpips_start_iters'] else 0,
            'sr_lip_lpips_loss': 0.5 * hparams['lambda_lpips_loss'] if self.global_step >= hparams[
                'lpips_start_iters'] else 0,
        ...
        try:
            ...
            grad_lambda_ambient = (math.log10(current_ambient_loss + 1e-15) - math.log10(
                target_ambient_loss + 1e-15))  # 如果需要增大lambda_ambient， 则current_loss大于targt，grad值大于0
        ...
        if (self.global_step + 1) % hparams['tb_log_interval'] == 0:
            density_grid_info = {
                ...
                "density_grid_info/occupancy_rate": (self.model.density_grid > min(self.model.mean_density,
                                                                                   self.model.density_thresh)).sum() / (
                                                                128 ** 3 * self.model.cascade),
    ...
    def validation_start(self):
        if self.global_step % hparams['valid_infer_interval'] == 0:
            self.gen_dir = os.path.join(hparams['work_dir'],
                                        f'validation_results/validation_{self.trainer.global_step}')
    ...
    def validation_step(self, sample, batch_idx):
        ...
        if self.global_step % hparams['valid_infer_interval'] == 0 \
            ...
            self.logger.add_figure(f"frame_{sample['idx']}/depth_pred", self.rgb_to_figure(depth_pred),
                                   self.global_step)
            ...
            if hparams['save_gt']:
                ...
                if self.global_step == hparams['valid_infer_interval']:
                    self.logger.add_figure(f"frame_{sample['idx']}/img_gt", self.rgb_to_figure(img_gt),
                                           self.global_step)
    ...
    ### GUI utils
    def test_gui_with_editable_data(self, pose, intrinsics, W, H, cond_wins, index=0, bg_color=None, spp=1,
                                    downscale=1):
    ...
    def test_gui_with_data(self, sample, target_W, target_H):
        ...
        preds = F.interpolate(preds.permute(0, 3, 1, 2), size=(target_H, target_W), mode='bilinear').permute(0, 2, 3,
                                                                                                             1).contiguous()
