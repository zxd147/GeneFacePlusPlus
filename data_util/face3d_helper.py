...
class Face3DHelper(nn.Module):
    ...
    def load_3dmm(self):
        ...
        self.register_buffer('mean_shape', torch.from_numpy(
            model['meanshape'].transpose()).float())  # mean face shape. [3*N, 1], N=35709, xyz=3, ==> 3*N=107127
        mean_shape = self.mean_shape.reshape([-1, 3])
        # re-center
        mean_shape = mean_shape - torch.mean(mean_shape, dim=0, keepdims=True)
        self.mean_shape = mean_shape.reshape([-1, 1])
        self.register_buffer('id_base', torch.from_numpy(
            model['idBase']).float())  # identity basis. [3*N,80], we have 80 eigen faces for identity
        self.register_buffer('exp_base', torch.from_numpy(
            model['exBase']).float())  # expression basis. [3*N,64], we have 64 eigen faces for expression

        self.register_buffer('mean_texure', torch.from_numpy(
            model['meantex'].transpose()).float())  # mean face texture. [3*N,1] (0-255)
        self.register_buffer('tex_base', torch.from_numpy(model['texBase']).float())  # texture basis. [3*N,80], rgb=3

        self.register_buffer('point_buf', torch.from_numpy(
            model['point_buf']).float())  # triangle indices for each vertex that lies in. starts from 1. [N,8] (1-F)
        self.register_buffer('face_buf', torch.from_numpy(
            model['tri']).float())  # vertex indices in each triangle. starts from 1. [F,3] (1-N)
        if self.keypoint_mode == 'mediapipe':
            self.register_buffer('key_points', torch.from_numpy(
                np.load("deep_3drecon/BFM/index_mp468_from_mesh35709.npy").astype(np.int64)))
            unmatch_mask = self.key_points < 0
            self.key_points[unmatch_mask] = 0
        else:
            self.register_buffer('key_points', torch.from_numpy(model['keypoints'].squeeze().astype(
                np.int_)).long())  # vertex indices of 68 facial landmarks. starts from 1. [68,1]
        ...
        self.register_buffer('key_exp_base',
                             self.exp_base.reshape([-1, 3, 64])[self.key_points, :, :].reshape([-1, 64]))
    ...
    def close_mouth_for_idexp_lm3d(self, idexp_lm3d, freeze_as_first_frame=True):
        ...
        idexp_lm3d[:, range(59, 54, -1), 1] = (idexp_lm3d[:, 49:54, 1] + idexp_lm3d[:, range(59, 54, -1),
                                                                         1]) / 2 - eps * 2
        ...
        idexp_lm3d[:, 49:54, 1] += (
                    0.03 - idexp_lm3d[:, 49:54, 1].mean(dim=1) + idexp_lm3d[:, 61:64, 1].mean(dim=1)).unsqueeze(
            1).repeat([1, 5])
        idexp_lm3d[:, range(59, 54, -1), 1] += (
                    -0.03 - idexp_lm3d[:, range(59, 54, -1), 1].mean(dim=1) + idexp_lm3d[:, range(67, 64, -1), 1].mean(
                dim=1)).unsqueeze(1).repeat([1, 5])
...
if __name__ == '__main__':
    ...
    lm3d = face_mesh_helper.reconstruct_lm2d(torch.tensor(coeff_dict['id']).cuda(),
