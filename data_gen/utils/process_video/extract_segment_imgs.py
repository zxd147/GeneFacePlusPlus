...
def extract_background(img_lst, segmap_mask_lst=None, method="knn", device='cpu', mix_bg=True):
    ...
    if method == "knn":
        ...
        if segmap_mask_lst is not None:
            segmap_mask_lst = segmap_mask_lst[
                              ::FRAME_SELECT_INTERVAL] if num_frames > FRAME_SELECT_INTERVAL else segmap_mask_lst[0:1]
...
def extract_segment_job(
        ...
        # set to True to speed up a bit of preprocess, but leads to HUGE memory costs (100GB for 5-min video)
    ...
    try:
        ...
        if multiprocess_enable:
            for (_, res) in multiprocess_run_tqdm(segment_and_generate_for_image_job, args=args, num_workers=16,
...
if __name__ == '__main__':
    ...
    fn_args = [(vid_name, ds_name == 'nerf', background_method, device, total_gpus, mix_bg, store_in_memory,
                force_single_process) for i, vid_name in enumerate(vid_names)]
