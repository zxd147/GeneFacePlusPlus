...
def cal_lan_loss_mp(proj_lan, gt_lan, mean: bool = True):
    ...
    eye = [33, 246, 161, 160, 159, 158, 157, 173, 133, 155, 154, 153, 145, 144, 163, 7] + [263, 466, 388, 387, 386, 385,
                                                                                           384, 398, 362, 382, 381, 380,
                                                                                           374, 373, 390, 249]
...
def fit_3dmm_for_a_video(
    ...
    if os.path.exists(lm_name):
        ...
        try:
            ...
            lms = combine_vid_img_lm478_to_lm478(img_lm478, vid_lm478)
    ...
    if id_mode == 'global':
        ...
        raise NotImplementedError(f"id mode {id_mode} not supported! we only support global or finegrained.")
    ...
    for _ in range(200):
        ...
        loss_lap = cal_lap_loss(id_para) + cal_lap_loss(exp_para) + cal_lap_loss(euler_angle) * 0.3 + cal_lap_loss(
            trans) * 0.3
    ...
    for i in range(int((num_frames - 1) / batch_size + 1)):
        ...
        for j in range(50):
            ...
            # loss_lap = cal_lap_loss(proj_geo)
            loss_lap = cal_lap_loss(sel_id_para) + cal_lap_loss(sel_exp_para) + cal_lap_loss(
                sel_euler_angle) * 0.3 + cal_lap_loss(sel_trans) * 0.3
        ...
        if debug:
            print(
                f"batch {i} | loss_lan: {loss_lan.item():.2f}, loss_reg_id: {loss_regid.item():.2f},loss_reg_exp: {loss_regexp.item():.2f},loss_lap_ldm:{loss_lap.item():.4f}")
    ...
    if debug:
        ...
        xy_camera_visualizer = CameraPoseVisualizer(
            xlim=[extrinsic[:, 0, 3].min().item() - 0.5, extrinsic[:, 0, 3].max().item() + 0.5],
            ylim=[extrinsic[:, 1, 3].min().item() - 0.5, extrinsic[:, 1, 3].max().item() + 0.5],
            zlim=[extrinsic[:, 2, 3].min().item() - 0.5, extrinsic[:, 2, 3].max().item() + 0.5], view_mode='xy')
        xz_camera_visualizer = CameraPoseVisualizer(
            xlim=[extrinsic[:, 0, 3].min().item() - 0.5, extrinsic[:, 0, 3].max().item() + 0.5],
            ylim=[extrinsic[:, 1, 3].min().item() - 0.5, extrinsic[:, 1, 3].max().item() + 0.5],
            zlim=[extrinsic[:, 2, 3].min().item() - 0.5, extrinsic[:, 2, 3].max().item() + 0.5], view_mode='xz')
        ...
        for i in tqdm.trange(min(250, len(frames)), desc=f'rendering debug video to {debug_name}..'):
            ...
            img = draw_axes(img, euler_angle[i, 0].item(), euler_angle[i, 1].item(), euler_angle[i, 2].item(),
                            lm68s[i][4][0].item(), lm68s[i, 4][1].item(), size=50)
...
if __name__ == '__main__':
    ...
    if ds_name.lower() == 'nerf':  # 处理单个视频
        vid_names = [vid_dir]
        out_names = [video_name.replace("/raw/", "/processed/").replace(".mp4", "_coeff_fit_mp.npy") for video_name in
                     vid_names]
    else:  # 处理整个数据集
        ...
        out_names = [video_name.replace("/video/", "/coeff_fit_mp/").replace(".mp4", "_coeff_fit_mp.npy") for video_name
                     in vid_names]
    ...
    for i in tqdm.trange(len(vid_names), desc=f"process {process_id}: fitting 3dmm ..."):
        ...
        try:
            ...
            success = fit_3dmm_for_a_video(img_name, is_person_specific_data, args.id_mode, args.debug,
                                           large_yaw_threshold=args.large_yaw_threshold)
            if not success:
            ...
            failed_img_names.append(img_name)
        print(
            f"finished {i + 1} / {len(vid_names)} = {(i + 1) / len(vid_names):.4f}, failed {len(failed_img_names)} / {i + 1} = {len(failed_img_names) / (i + 1):.4f}")
        sys.stdout.flush()
